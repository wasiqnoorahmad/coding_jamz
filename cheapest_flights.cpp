...
to n - 1. The size of flights will be in range [0, n * (n - 1) / 2]. The format
...
void dfs(vector<vector<int>> &adj, int src, int dst, int K, int cost,
         int &min_cost) {
  // When recusrion ends, we should have the cost to reach dst from all childs
  // of src ... Either cost will have some value, or it should be MAX
  ...
  for (int i = 0; i < adj.size(); i++) {
    if (adj[src][i] != -1) {
      dfs(adj, i, dst, K - 1, cost + adj[src][i], min_cost);
    }
  }
}

int find_cheapest_price(int n, vector<vector<int>> &flights, int src, int dst,
                        int K) {
  /* Prepare Adj matrix */
  vector<vector<int>> adj(n, vector<int>(n, -1));
  for (vector<int> &path : flights)
    adj[path[0]][path[1]] = path[2];
  int min_cost = INT_MAX;
  dfs(adj, src, dst, K + 1, 0, min_cost);
  if (min_cost == INT_MAX)
    return -1;
  return min_cost;
}
